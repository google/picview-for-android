...
public class CachedWebRequestFetcher {
	 ...
	 * (file-system or in-memory), this version is returned. If the response
	 * could not be found in cache, it's fetched and automatically put into both
	 * caches.
	 * 
	 * @param url
	 *            the URL to fetch
	 * @param forceFetchFromWeb
	 *            whether the content should be fetched from the web, regardless
	 *            of whether it is present in any of the caches
	 */
		...
		synchronized (url) {
			...
			if (!forceFetchFromWeb) {
				...
				if (cache.containsKey(url) && cache.get(url) != null) {
					return new CachedResponse<String>(
							CachedResponse.FROM_MEMORY, cache.get(url));
				}
			...
			return new CachedResponse<String>(
					fromDatabase ? CachedResponse.FROM_FILE
							: CachedResponse.NOT_CACHED, responseText);
		}
	}

	/**
	 * Returns whether a response from a request with the given URL exists in
	 * the in-memory cache.
	 */

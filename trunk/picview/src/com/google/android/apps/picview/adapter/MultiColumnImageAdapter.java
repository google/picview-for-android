...
public abstract class MultiColumnImageAdapter<T> extends BaseAdapter {
	 ...
	 *            used to determine who many thumbnails can be display on a
	 *            single row
	 */
	public MultiColumnImageAdapter(List<ThumbnailItem<T>> dataItems,
		...
		slotsPerRow = (int) Math.floor(displayMetrics.widthPixels
				/ thumbnailWithPx);
	...
	/**
	 * Recycles a slot view, if it already exists. This means, changing the
	 * title, stopping a previous image loading task and instantiating a new
	 * image loading task.
	 * 
	 * @param slot
	 *            the slot to recycle
	...
	private void recycleSlotView(ThumbnailSlotView slot,
		...
		try {
			ImageLoadingTask task = new ImageLoadingTask(albumThumbnail,
					new URL(item.getThumbnailUrl()), cachedImageFetcher);

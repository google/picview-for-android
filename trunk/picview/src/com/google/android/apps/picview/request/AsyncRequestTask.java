...
public class AsyncRequestTask extends AsyncTask<Void, Integer, String> {
	...
	protected String doInBackground(Void... params) {
		try {
			CachedResponse<String> cachedResponse = fetcher.cachedFetch(
					new URL(url), forceFetchFromWeb);
	...
	/**
	 * Fire off another {@link AsyncRequestTask} that forces the fetcher to
	 * fetch the contents from the web and skip the caches. This way we can
	 * check, whether there is updated data available. If there is, then the
	 * callback we be called once again with the updated data.
	 * 
	 * @param oldData
	 *            the old data, so we can compare the new data. Only if new and
	 *            old differ, we call the callback once again
	 */
